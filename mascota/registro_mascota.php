...
$error=false;
$db_foto_path = null; // ruta pública relativa para guardar en la BD
...
if(isset($_POST['btn_enviar'])){
    ...
    if (!$error) {
      ...
      // Procesar la imagen
          ...
          // Crear directorio absoluto si no existe (filesystem)
          $upload_dir_fs = rtrim(ROOT_PATH, DIRECTORY_SEPARATOR) . '/assets/images/mascotas/';
          ...
          // Guardar en DB una ruta relativa sin '../' para servir via BASE_URL
          $db_foto_path = 'assets/images/mascotas/' . $filename;
  ...
  $sql = "INSERT INTO mascotas (nombre, especie, edad, raza, color, sexo, id, foto_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($result) {
          ...
          // 🔥 GENERAR CÓDIGO QR AUTOMÁTICAMENTE (best-effort)
          ...
          header('Location: ' . BASE_URL . '/mascota/perfil_mascota.php?id=' . $mascota_id);
          exit;
        } else {
  ...
  <section class="registro-mascota">
    <form class="formulario" method="post" action="" enctype="multipart/form-data">
      ...
      <div class="form-header">
        <button type="button" onclick="window.location.href='../usuario/perfil_usuario.php'" class="btn-back-arrow">
          ← 
        </button>
